...
function DayCard({
  ...
  useEffect(() => {
    ...
    fetchPlaceImage(day.theme + ' ' + day.activities[0]?.name || day.theme).then((url) => {
      if (!cancelled) {
        setImageUrl(url);
                      ...
                      {isSendingPDF ? (
                        <>
                          <motion.div
                            ...
                            style={{ width: 14, height: 14, border: '1.5px solid rgba(196,163,90,0.3)', borderTopColor: '#c4a35a', borderRadius: '50%' }}
                          />
                          Generating PDF...
                        </>
                      ) : (
                        <>
                          <Mail size={14} />
                          {user ? 'Send PDF to My Email' : 'Email Plan as PDF'}
                        </>
                      )}
                    </button>
                  ) : (
